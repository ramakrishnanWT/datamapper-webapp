...
<style>
  ...
  #dxm-modal .mb{padding:18px;overflow-y:auto;flex:1;}
  ...
  #dxm-modal label.field-lbl{display:block;font-size:0.78rem;font-weight:600;color:#444;margin-bottom:4px;}
  ...
  #dxm-modal textarea{width:100%;padding:7px 9px;border:1px solid #ccc;border-radius:5px;
    font-size:0.75rem;font-family:'Cascadia Code','Fira Code',monospace;resize:vertical;box-sizing:border-box;}
  #dxm-modal .btn-ok{background:#0078d4;color:#fff;border:none;border-radius:6px;padding:7px 16px;cursor:pointer;font-size:0.85rem;font-weight:600;}
  #dxm-modal .btn-ok:hover{background:#006cbe;}
  #dxm-modal .btn-ok:disabled{background:#8ab8e0;cursor:default;}
  #dxm-modal .btn-cn{background:#e0e1e5;color:#222;border:none;border-radius:6px;padding:7px 14px;cursor:pointer;font-size:0.85rem;}
  #dxm-snap-status{font-size:0.78rem;color:#666;margin-top:10px;display:flex;align-items:center;gap:6px;}
  #dxm-snap-status .dot{width:8px;height:8px;border-radius:50%;background:#ccc;flex-shrink:0;}
  #dxm-snap-status .dot.ok{background:#2a7a2a;}
  #dxm-snap-status .dot.empty{background:#e0a000;}
  #dxm-snap-status .dot.loading{background:#0078d4;animation:dxm-pulse 1s infinite;}
  @keyframes dxm-pulse{0%,100%{opacity:1}50%{opacity:.3}}
  /* collapsible review panel */
  #dxm-review-toggle{background:none;border:none;color:#0078d4;cursor:pointer;font-size:0.78rem;
    padding:0;text-decoration:underline;text-underline-offset:2px;margin-top:12px;display:block;}
  #dxm-review-panel{display:none;margin-top:10px;border-top:1px solid #eee;padding-top:10px;}
  #dxm-review-panel.open{display:block;}
  .dxm-rf{margin-bottom:10px;}
  .dxm-rf-hdr{display:flex;align-items:center;gap:8px;margin-bottom:3px;}
  .dxm-rf-hdr label{margin:0;font-size:0.76rem;font-weight:600;color:#444;flex:1;}
  .dxm-upload-btn{font-size:0.7rem;padding:2px 8px;background:#e8f0fe;color:#0078d4;
    border:1px solid #c0d4f5;border-radius:4px;cursor:pointer;white-space:nowrap;}
  .dxm-upload-btn:hover{background:#d0e4fc;}
  .dxm-fname{font-size:0.68rem;color:#888;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
  #dxm-msg{font-size:0.8rem;margin-top:8px;}
  #dxm-msg.ok{color:#2a7a2a;} #dxm-msg.err{color:#c00;}
</style>
...
<div id="dxm-modal-bg">
  <div id="dxm-modal">
    ...
    <div class="mb">
      <label class="field-lbl">Map Name *</label>
      <input type="text" id="dxm-name" placeholder="e.g. order-to-invoice-v1" autocomplete="off">

      <div id="dxm-snap-status">
        <span class="dot loading" id="dxm-dot"></span>
        <span id="dxm-snap-text">Reading current workspace state&hellip;</span>
      </div>
      ...
      <button id="dxm-review-toggle" onclick="dxmToggleReview()">&#9660; Review / override captured content</button>

      <div id="dxm-review-panel">
        <div class="dxm-rf">
          <div class="dxm-rf-hdr">
            <label>Input Schema</label>
            <label class="dxm-upload-btn">&#128194; Upload<input type="file" accept=".json,.schema.json,.xsd,.xml" style="display:none" onchange="dxmReadFile(this,'dxm-in-ta','dxm-in-fn','dxm-in')"></label>
            <span class="dxm-fname" id="dxm-in-fn"></span>
          </div>
          <textarea id="dxm-in-ta" rows="4" oninput="dxmSync('dxm-in-ta','dxm-in')"></textarea>
        </div>
        <div class="dxm-rf">
          <div class="dxm-rf-hdr">
            <label>Output Schema</label>
            <label class="dxm-upload-btn">&#128194; Upload<input type="file" accept=".json,.schema.json,.xsd,.xml" style="display:none" onchange="dxmReadFile(this,'dxm-out-ta','dxm-out-fn','dxm-out')"></label>
            <span class="dxm-fname" id="dxm-out-fn"></span>
          </div>
          <textarea id="dxm-out-ta" rows="4" oninput="dxmSync('dxm-out-ta','dxm-out')"></textarea>
        </div>
        <div class="dxm-rf">
          <div class="dxm-rf-hdr">
            <label>Map Content (.dmf / XSLT)</label>
            <label class="dxm-upload-btn">&#128194; Upload<input type="file" accept=".xsl,.xslt,.dmf,.yaml" style="display:none" onchange="dxmReadFile(this,'dxm-map-ta','dxm-map-fn','dxm-map')"></label>
            <span class="dxm-fname" id="dxm-map-fn"></span>
          </div>
          <textarea id="dxm-map-ta" rows="5" oninput="dxmSync('dxm-map-ta','dxm-map')"></textarea>
        </div>
      </div>
...
(function(){
  /* ── helpers ─────────────────────────────────────────────────────── */
  function dxmSync(taId, hiddenId){
    document.getElementById(hiddenId).value = document.getElementById(taId).value;
  }

  window.dxmReadFile = function(input, taId, fnameId, hiddenId){
    const file = input.files[0];
    if(!file) return;
    document.getElementById(fnameId).textContent = file.name;
  ...
  function setSnapStatus(state, text){
    const dot = document.getElementById('dxm-dot');
    dot.className = 'dot ' + state;
    document.getElementById('dxm-snap-text').textContent = text;
  }

  /* ── snapshot load ───────────────────────────────────────────────── */
  async function dxmLoadSnapshot(){
    setSnapStatus('loading', 'Reading current workspace state\u2026');
    try{
      const r = await fetch('/api/workspace-snapshot');
      if(!r.ok) throw new Error('HTTP ' + r.status);
      const d = await r.json();
      const inVal  = d.input_schema  || '';
      const outVal = d.output_schema || '';
      const mapVal = d.map_content   || '';
      document.getElementById('dxm-in').value  = inVal;
      document.getElementById('dxm-out').value = outVal;
      document.getElementById('dxm-map').value = mapVal;
      // also keep review textareas in sync
      document.getElementById('dxm-in-ta').value  = inVal;
      document.getElementById('dxm-out-ta').value = outVal;
      document.getElementById('dxm-map-ta').value = mapVal;

      const captured = [
        inVal  && 'input schema',
        outVal && 'output schema',
        mapVal && 'map content',
      ].filter(Boolean);
      ...
      setSnapStatus('empty', 'Could not read workspace (' + e.message + '). Expand below to add content manually.');
      document.getElementById('dxm-review-panel').classList.add('open');
    }
  }

  /* ── review panel toggle ─────────────────────────────────────────── */
  window.dxmToggleReview = function(){
    const panel  = document.getElementById('dxm-review-panel');
    const toggle = document.getElementById('dxm-review-toggle');
    const open   = panel.classList.toggle('open');
    toggle.textContent = (open ? '\u25b2' : '\u25bc') + ' Review / override captured content';
  };
  ...
  window.dxmOpenSave = function(){
    ...
    ['dxm-in-ta','dxm-out-ta','dxm-map-ta'].forEach(id => document.getElementById(id).value = '');
    ['dxm-in-fn','dxm-out-fn','dxm-map-fn'].forEach(id => document.getElementById(id).textContent = '');
    document.getElementById('dxm-review-panel').classList.remove('open');
    document.getElementById('dxm-review-toggle').textContent = '\u25bc Review / override captured content';
