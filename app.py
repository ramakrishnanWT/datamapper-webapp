...
<style>
  ...
  #dxm-modal{background:#fff;border-radius:10px;width:660px;max-width:95vw;max-height:90vh;
  ...
  #dxm-modal .mf{padding:12px 18px;border-top:1px solid #eee;display:flex;gap:10px;justify-content:flex-end;}
  #dxm-modal label{display:block;font-size:0.78rem;font-weight:600;color:#444;margin-bottom:3px;}
  #dxm-modal input[type=text]{width:100%;padding:7px 9px;border:1px solid #ccc;border-radius:5px;font-size:0.88rem;margin-bottom:12px;}
  #dxm-modal textarea{width:100%;padding:7px 9px;border:1px solid #ccc;border-radius:5px;
    font-size:0.78rem;font-family:'Cascadia Code','Fira Code',monospace;resize:vertical;}
  ...
  #dxm-modal .hint{font-size:0.75rem;color:#777;margin-bottom:10px;}
  #dxm-msg{font-size:0.8rem;margin-top:8px;}
  #dxm-msg.ok{color:#2a7a2a;} #dxm-msg.err{color:#c00;}
  .dxm-upload-row{display:flex;align-items:center;gap:8px;margin-bottom:3px;}
  .dxm-upload-row label.field-lbl{margin:0;flex:1;font-size:0.78rem;font-weight:600;color:#444;}
  .dxm-upload-btn{font-size:0.72rem;padding:3px 9px;background:#e8f0fe;color:#0078d4;
    border:1px solid #c0d4f5;border-radius:5px;cursor:pointer;white-space:nowrap;}
  .dxm-upload-btn:hover{background:#d0e4fc;}
  .dxm-fname{font-size:0.7rem;color:#777;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
</style>
...
<div id="dxm-modal-bg">
  <div id="dxm-modal">
    ...
    <div class="mb">
      ...
      <input type="text" id="dxm-name" placeholder="e.g. order-to-invoice-v1">
      <p class="hint">Workspace files are auto-loaded below. You can also upload files or paste content directly.</p>
      <div class="dxm-upload-row">
        <label class="field-lbl">Input Schema</label>
        <label class="dxm-upload-btn" title="Upload JSON Schema (.json) or XML Schema (.xsd)">&#128194; Upload<input type="file" accept=".json,.schema.json,.xsd,.xml" style="display:none" onchange="dxmRead(this,'dxm-in','dxm-in-fn')"></label>
        <span class="dxm-fname" id="dxm-in-fn"></span>
      </div>
      <textarea id="dxm-in" rows="5" placeholder="Paste JSON Schema / XSD or upload above..."></textarea>
      <div class="dxm-upload-row" style="margin-top:10px">
        <label class="field-lbl">Output Schema</label>
        <label class="dxm-upload-btn" title="Upload JSON Schema (.json) or XML Schema (.xsd)">&#128194; Upload<input type="file" accept=".json,.schema.json,.xsd,.xml" style="display:none" onchange="dxmRead(this,'dxm-out','dxm-out-fn')"></label>
        <span class="dxm-fname" id="dxm-out-fn"></span>
      </div>
      <textarea id="dxm-out" rows="5" placeholder="Paste JSON Schema / XSD or upload above..."></textarea>
      <div class="dxm-upload-row" style="margin-top:10px">
        <label class="field-lbl">Map Content (XSLT / .dmf)</label>
        <label class="dxm-upload-btn" title="Upload .xsl or .dmf">&#128194; Upload<input type="file" accept=".xsl,.xslt,.dmf,.yaml" style="display:none" onchange="dxmRead(this,'dxm-map','dxm-map-fn')"></label>
        <span class="dxm-fname" id="dxm-map-fn"></span>
      </div>
      <textarea id="dxm-map" rows="7" placeholder="Paste XSLT/DMF or upload above..."></textarea>
      <div id="dxm-msg"></div>
    </div>
    <div class="mf">
      <button class="btn-cn" onclick="dxmClose()">Cancel</button>
      <button class="btn-ok" onclick="dxmSave()">Save to DuckDB</button>
    </div>
  </div>
</div>
...
<script>
(function(){
  function dxmReadFile(input, textareaId, fnameId){
    const file = input.files[0];
    if(!file) return;
    document.getElementById(fnameId).textContent = file.name;
    const r = new FileReader();
    r.onload = e => { document.getElementById(textareaId).value = e.target.result; };
    r.readAsText(file);
  }
  window.dxmRead = dxmReadFile;
  ...
  async function dxmLoadSnapshot(){
    try{
      ...
      document.getElementById('dxm-in').value  = d.input_schema  || '';
      document.getElementById('dxm-out').value = d.output_schema || '';
      document.getElementById('dxm-map').value = d.map_content   || '';
    }catch(e){console.warn('DXM snapshot load failed',e);}
  }
  window.dxmOpenSave=function(){
    document.getElementById('dxm-name').value='';
    document.getElementById('dxm-in').value='';
    document.getElementById('dxm-out').value='';
    document.getElementById('dxm-map').value='';
  ...
  window.dxmSave=async function(){
    ...
    try{
      ...
      if(!res.ok)throw new Error(await res.text());
      const d=await res.json();
      msg.className='ok';msg.textContent='Saved \u2713  (ID '+d.id+')';
      setTimeout(()=>dxmClose(),900);
    }catch(e){msg.className='err';msg.textContent='Error: '+e.message;}
  };
